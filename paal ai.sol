...
contract PAALAI is IERC20 {
    ...
    string private constant _name = "PAAL AI";
    string private constant _symbol = "$PAAL";
    ...
    Fees public _taxRates = Fees({buyFee: 1000, sellFee: 1000, transferFee: 0});
    ...
    uint256 public lastUpdatedBuyTax;
    uint256 public lastUpdatedSellTax;
    ...
    constructor() payable {
        ...
        lastUpdatedBuyTax = block.timestamp;
        lastUpdatedSellTax = block.timestamp;
    ...
    function takeTaxes(
        ...
        uint256 currentFee;
        if (buy) {
            if (_taxRates.buyFee != 500) {
                uint256 timeStamp = lastUpdatedBuyTax + 30 minutes;
                _taxRates.buyFee = block.timestamp >= timeStamp
                    ? _taxRates.buyFee - 100
                    : _taxRates.buyFee;
                lastUpdatedBuyTax = block.timestamp >= timeStamp
                    ? block.timestamp
                    : lastUpdatedBuyTax;
            }
            ...
            if (_taxRates.sellFee != 500) {
                uint256 timeStamp = lastUpdatedSellTax + 30 minutes;
                _taxRates.sellFee = block.timestamp >= timeStamp
                    ? _taxRates.sellFee - 100
                    : _taxRates.sellFee;
                lastUpdatedSellTax = block.timestamp >= timeStamp
                    ? block.timestamp
                    : lastUpdatedSellTax;
            }
